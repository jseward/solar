...
namespace solar {
	...
	d3d9_texture::d3d9_texture(d3d9_texture_factory& factory, const resource_address& address, const texture_create_params& create_params)
		: _factory(factory)
		, _address(address)
		, _create_params(create_params)
	...
	const resource_address& d3d9_texture::get_address() const {
		return _address;
	}

	const texture_create_params& d3d9_texture::get_create_params() const {
	...
	void d3d9_texture::create_IDirect3DTexture9(IDirect3DDevice9* device) {
		...
		if (_factory.get_resource_system().read_to_mapped_memory(resource_mapped_memory, _address)) {

			unsigned int mip_levels = D3DX_DEFAULT;
			if (!_create_params._has_mip_maps) {
				mip_levels = 1;
			}
			...
			HRESULT hr = ::D3DXCreateTextureFromFileInMemoryEx(
				...
				&_IDirect3DTexture9);
			if (FAILED(hr)) {
				ALERT("Failed to create texture.\n\naddress: {}\nerror: {}", _address, make_d3d9_error_message(hr));
			}
		}
