...
namespace solar {
	...
	win32_opengl_context::win32_opengl_context()
		: _hwnd(nullptr)
	...
	bool win32_opengl_context::setup(HWND hwnd, const opengl_user_settings& settings) {
		_hwnd = hwnd;
	...
	void win32_opengl_context::teardown() {
		//todo
		ASSERT(false);
	}

	void win32_opengl_context::add_backbuffer_size_changed_callback(backbuffer_size_changed_callback callback) {
		_backbuffer_size_changed_callbacks.push_back(callback);
	}

	bool win32_opengl_context::apply_user_settings(const opengl_user_settings& new_settings) {
		ASSERT(_hwnd != nullptr);

		TRACE("win32_opengl_context apply user settings...");
		new_settings.trace_dump();

		opengl_user_settings old_settings = _user_settings;
		_user_settings = new_settings;

		DWORD new_style = 0;
		size client_size;

		if (new_settings.get_window_type() == opengl_window_type::RESIZABLE_WINDOW) {
			new_style = WS_OVERLAPPEDWINDOW | WS_SYSMENU;
			client_size = new_settings.get_window_client_size();
		}
		else if (new_settings.get_window_type() == opengl_window_type::VIRTUAL_FULLSCREEN) {
		...
		reset_opengl(new_window_rect.get_size() - size(non_client_width, non_client_height));

		return true;
	}

	void win32_opengl_context::attempt_render_scene(render_scene_functor& func) {
		bool is_paused =
	...
	void win32_opengl_context::reset_opengl(const size& new_backbuffer_size) {
	...
	bool win32_opengl_context::handle_wnd_proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
	...
	void win32_opengl_context::check_if_window_size_changed() {
		bool should_check = 
			_user_settings.get_window_type() == opengl_window_type::RESIZABLE_WINDOW &&
			!_is_window_minimized &&
			!_is_window_in_size_move;

		if (should_check) {
			if (get_window_client_size() != _backbuffer_size) {
				reset_opengl(get_window_client_size());
			}
		}
	}

	void win32_opengl_context::toggle_virtual_fullscreen() {

		if (_user_settings.get_window_type() == opengl_window_type::VIRTUAL_FULLSCREEN) {
			auto new_settings = _user_settings;
			new_settings.set_window_type(opengl_window_type::RESIZABLE_WINDOW);
			apply_user_settings(new_settings);
		}
		else if (_user_settings.get_window_type() == opengl_window_type::RESIZABLE_WINDOW) {
			auto new_settings = _user_settings;
			new_settings.set_window_type(opengl_window_type::VIRTUAL_FULLSCREEN);
	...
	size win32_opengl_context::get_backbuffer_size() const {
		return _backbuffer_size;
	}

	size win32_opengl_context::get_window_client_size() const {
		RECT client_rect;
	...
	size win32_opengl_context::get_desktop_size() const {
		return size(
