...
namespace solar {
	...
	win32_single_directory_change_watcher::win32_single_directory_change_watcher(const std::string& dir_path, bool is_recursive)
		: _dir_path(dir_path)
		, _is_recursive(is_recursive) {
	}

	win32_single_directory_change_watcher::~win32_single_directory_change_watcher() {
		ASSERT(_reading_thread == nullptr);
	}

	const std::string& win32_single_directory_change_watcher::get_dir_path() const {
		return _dir_path;
	}

	void win32_single_directory_change_watcher::begin_watching() {
		TRACE("watching directory for changes '{}' (is_recursive:{})", _dir_path, bool_to_string(_is_recursive));

		_h_dir = ::CreateFile(
			utf8_to_utf16(_dir_path).c_str(),
			FILE_LIST_DIRECTORY,
		...
		if (_h_dir.get() == INVALID_HANDLE_VALUE) {
			ALERT("win32_single_directory_change_watcher creating handle to directory failed.\ndir_path : '{}'\nerror : {}", _dir_path, get_last_win32_error_message());
		}
		else {

			_h_iocp = ::CreateIoCompletionPort(_h_dir.get(), NULL, 0, 1);
			if (_h_iocp.get() == NULL) {
				ALERT("win32_single_directory_change_watcher creating IoCompletionPort failed.\nerror : {}", get_last_win32_error_message());
			}
			else {

				ASSERT(_reading_thread == nullptr);
				_reading_thread = std::unique_ptr<std::thread>(new std::thread(&win32_single_directory_change_watcher::run_in_thread, this, _h_iocp.get(), _h_dir.get(), _is_recursive));

			}
		}
	}

	void win32_single_directory_change_watcher::end_watching() {
	...
	bool win32_single_directory_change_watcher::try_pop_directory_change(directory_change& change) {
	...
	void win32_single_directory_change_watcher::try_complete_staged_changes() {
	...
	void win32_single_directory_change_watcher::run_in_thread(HANDLE h_iocp, HANDLE h_dir, bool is_recursive) {

		set_current_thread_name("win32_single_directory_change_watcher");
		...
		for (;;) {
			...
			BOOL read_result = ::ReadDirectoryChangesW(
				...
				NULL);
			if (read_result == 0) {
				ALERT("win32_single_directory_change_watcher ReadDirectoryChangesW failed.\nerror : {}", get_last_win32_error_message());
				break;
			}
			...
			if (get_status_result == 0) {
				ALERT("win32_single_directory_change_watcher GetQueuedCompletionStatus failed.\nerror : {}", get_last_win32_error_message());
				break;
			}
			else {
	...
	void win32_single_directory_change_watcher::handle_notifications_in_thread(const char* buffer, DWORD size) {
	...
	void win32_single_directory_change_watcher::handle_notification_in_thread(const FILE_NOTIFY_INFORMATION* fni) {
